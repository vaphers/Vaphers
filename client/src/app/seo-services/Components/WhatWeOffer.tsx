  ...
  return (
    <section className="bg-[#FAFAFA] py-12 sm:py-14 px-4 sm:px-6 md:px-12 lg:px-24">
      <div className="max-w-[1400px] mx-auto">
        ...
        <h3 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-medium tracking-tight text-gray-900 mb-10 sm:mb-16 bungee-shade leading-[1.2]">
          What You Get At <br className="block sm:hidden" /> <span className='text-blue-700'>Vaphers?</span>
        </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 pb-4 lg:pb-12">
          {services.map((service, index) => (
            <Link 
              key={index} 
              href={service.link}
              // {/* Fluid padding, responsive minimum heights, and rounded corners */}
            ...
            >
              ...
              <div className="mt-auto flex flex-wrap gap-2 pt-2 sm:pt-4">
                {service.tags.map((tag, tagIndex) => (
                  <span 
                    key={tagIndex} 
                    // {/* Tags scaled down slightly on mobile so multiple fit on one line */}
