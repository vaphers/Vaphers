...
// const METRICS = [
//   { id: 'sessions', label: 'Sessions', isTime: false, isPercent: false },
//   { id: 'engagedSessions', label: 'Engaged Sessions', isTime: false, isPercent: false },
//   { id: 'newUsers', label: 'New Users', isTime: false, isPercent: false },
//   { id: 'users', label: 'Users', isTime: false, isPercent: false },
//   { id: 'views', label: 'Views', isTime: false, isPercent: false },
//   { id: 'avgEngagementTime', label: 'Avg. Engagement Time', isTime: true, isPercent: false },
//   { id: 'engagementRate', label: 'Engagement Rate', isTime: false, isPercent: true },
//   { id: 'conversions', label: 'Conversions', isTime: false, isPercent: false }
// ];
...
//       // We must track the sum of (Rate * Sessions) to calculate true weighted averages later
//       let totalEngagementTimeSum = 0;
//       let totalEngagementRateSum = 0;

//       dataset.forEach(row => {
//         // Enforce strong number types
...
//         const rowAvgTime = parseFloat(row.avgEngagementTime) || 0;
...
//         // Accumulate Weighted Sums for Global Totals
//         totalEngagementTimeSum += (rowAvgTime * sessions);
//         totalEngagementRateSum += (rowEngRate * sessions);

//         // --- 1. Process Country Data ---
...
//           // Accumulate Weighted Sums for this specific country
//           countryMap[normalizedCountry]._timeSum += (rowAvgTime * sessions);
//           countryMap[normalizedCountry]._rateSum += (rowEngRate * sessions);
//         }

//         // --- 2. Process City Data ---
//         const cityKey = `${row.city}_${row.country}`; // Prevent mixing up cities with same names
//         if (!cityMap[cityKey]) {
//             cityMap[cityKey] = { 
//               ...row,
//               sessions, engagedSessions, newUsers, users, views, conversions,
...
//             // Accumulate Weighted Sums for this specific city
//             cityMap[cityKey]._timeSum += (rowAvgTime * sessions);
//             cityMap[cityKey]._rateSum += (rowEngRate * sessions);
//         }
//       });

//       // Calculate True Global Averages
//       totals.avgEngagementTime = totals.sessions > 0 ? (totalEngagementTimeSum / totals.sessions) : 0;
//       totals.engagementRate = totals.sessions > 0 ? (totalEngagementRateSum / totals.sessions) : 0;

//       // Calculate True Row Averages
//       const finalizeAverages = (mapObj: Record<string, any>) => {
//         return Object.values(mapObj).map(item => {
//           return {
//             ...item,
//             avgEngagementTime: item.sessions > 0 ? (item._timeSum / item.sessions) : 0,
...
//     // Find max value for map opacity
...
//   const renderDelta = (currentVal: number, compareVal: number | undefined, isPercent: boolean, isTime: boolean) => {
...
//       : isTime ? Math.abs(diff).toFixed(0) : Math.abs(diff).toLocaleString();

//     return (
//       <span className={`text-xs font-semibold ${color} ml-2 flex items-center`}>
//         {arrow} {formattedDiff}{isPercent ? '%' : ''}{isTime ? 's' : ''}
//       </span>
//     );
//   };
...
//           else if (m.isTime) displayVal = `${val.toFixed(0)}s`;
...
//                 {displayVal}
//                 {compared && renderDelta(val, compVal, m.isPercent, m.isTime)}
//               </div>
//             </button>
//           );
//         })}
...
//                              const opacity = 0.2 + (0.8 * (value / maxMetricValue));
//                              fillColor = `rgba(99, 102, 241, ${opacity})`; 
//                          }
...
//                                const value = countryData[activeMetric];
//                                const opacity = 0.2 + (0.8 * (value / maxMetricValue)); 
//                                fillColor = `rgba(234, 179, 8, ${opacity})`; 
//                            }
...
//                                style={{
//                                  default: { outline: "none" },
//                                  hover: { fill: "#ca8a04", outline: "none" },
//                                }}
//                              />
//                            )
//                          })
...
//                   <td className="px-6 py-4">
//                      <div className="flex items-center">
//                       {Number(row.avgEngagementTime).toFixed(0)}s
//                       {compRow && renderDelta(Number(row.avgEngagementTime), Number(compRow.avgEngagementTime), false, true)}
//                     </div>
//                   </td>
//                   <td className="px-6 py-4">
//                      <div className="flex items-center">
