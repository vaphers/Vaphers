  ...
  return (
    <NodeViewWrapper as="figure" className="relative group my-8 flex flex-col items-center max-w-full">
      ...
      <input
        ...
        // Prevents Tiptap from hijacking backspace or enter key while typing in the caption box
...
const CustomImage = Image.extend({
  // THESE ENSURE TIPTAP SAVES IT AS A VALID HTML FIGURE BLOCK, NOT STRIPPED OUT BY PARAGRAPHS
  inline: false, 
  group: 'block',
  draggable: true,
  ...
  parseHTML() {
    return [
      ...
      { tag: 'img' } // Fallback for standard images without figure wrapper
    ];
  },
  renderHTML({ HTMLAttributes }) {
    const { caption, style, float, width, height, ...imgAttrs } = HTMLAttributes;
    const figureStyle = `${style || ''} float: ${float || 'none'}; width: ${width || 'auto'}; height: ${height || 'auto'};`.trim();
    
    // Outputs the figure/figcaption HTML to be saved to DB
    if (caption) {
      return ['figure', { class: 'image-wrapper', style: figureStyle }, 
...
const Tiptap: React.FC<EditorProps> = ({ content, onChange, title, onTitleChange }) => {
  ...
  const editor = useEditor({
    extensions: [
      ...
      CustomCodeBlock.configure({ lowlight, defaultLanguage: 'javascript' }),
      FaqBox, CtaBox, TestimonialBox,
      CustomImage, // <-- Replaced standard Image with CustomImage
      TextAlign.configure({ types: ['heading', 'paragraph', 'image'], alignments: ['left', 'center', 'right', 'justify'] }),
      Link.configure({ openOnClick: false, HTMLAttributes: { class: 'text-blue-600 underline underline-offset-2 decoration-[#d3e3fd] hover:decoration-blue-600 transition-colors', target: '_blank', rel: 'noopener noreferrer' } }),
      Underline, Blockquote, Table.configure({ resizable: true }), TableRow, TableCell, TableHeader, HardBreak, 
    ],
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        alert(`Upload Failed: ${json.error?.message || '401 Unauthorized'}\nMake sure your Upload Preset is set to "Unsigned".`);
        return;
      }
