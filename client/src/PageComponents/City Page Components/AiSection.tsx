...
const AiSection: React.FC<AiSectionProps> = ({ cityName }) => {
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            <div className="space-y-6">
              <p className="text-lg sm:text-lg lg:text-xl text-gray-700 leading-relaxed">{`Rapid advances in generative artificial intelligence (AI) models have reshaped how content is discovered and consumed. Traditional search engine optimization (SEO) is no longer enough to reach those looking for your products or services in ${cityName}. To stay visible online amid shifting algorithms, you need to optimize for AI-first discovery.`}</p>
              <p className="text-lg sm:text-lg lg:text-xl text-gray-700 leading-relaxed">
                Leveraging the latest in artificial intelligence, our SEO strategies go beyond traditional optimization techniques. We tailor your content to align with AI-powered search algorithms, ensuring that your business stands out in voice search, semantic search, and AI-driven query results. 
              </p>
                ...
                {tabContents.map((tab) => (
                  <button
                    ...
                    className={`w-14 h-14 rounded-full flex items-center justify-center transition-all duration-300 transform hover:scale-110 ${
                      activeTab === tab.id
                        ? tab.id === 'visibility'
                          ? 'bg-blue-600 shadow-blue-200 ring-4 ring-blue-100 shadow-lg'
                          : tab.id === 'search'
                          ? 'bg-teal-600 shadow-teal-200 ring-4 ring-teal-100 shadow-lg'
                          : tab.id === 'refresh'
                          ? 'bg-green-600 shadow-green-200 ring-4 ring-green-100 shadow-lg'
                          : 'bg-purple-600 shadow-purple-200 ring-4 ring-purple-100 shadow-lg'
                        : tab.id === 'visibility'
