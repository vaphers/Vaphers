...
export default function CityPageFaq({
  data,
  heading = "FAQs About SEO Services",
  subheading = "Clear answers about SEO pricing, timelines, process, ownership, and maintenance so you know exactly what to expect.",
  defaultOpen = [0],
  cityName = "",
}: CityPageFaqProps) {
  const [value, setValue] = useState<string>();
  const items = (data?.length ? data : defaultFaq).map(({ question, answer }) => ({
