...
const ContactForm: React.FC = () => {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    fd.set('access_key', process.env.NEXT_PUBLIC_WEB3FORMS_ACCESS_KEY || '') // public client key [web:9][web:12]
    fd.set('from_name', 'Website Contact Form') // optional label in email [web:44]
    fd.set('subject', 'New contact via website') // optional subject [web:53]
    if (service) fd.set('service', service)
      ...
      try {
        ...
        if (data.success) {
          toast('Message sent', {
            description: 'Thanks! Your message reached the inbox.',
            duration: 5000, // instead of important
          }) // [web:102]
          form.reset()
          ...
          toast('Submission failed', {
            ...
            duration: 7000, // keep visible longer
          }) // [web:102]
        }
      } catch {
        toast('Network error', {
          description: 'Could not send your message. Retry later.',
          duration: 7000,
        }) // [web:102]
      }
      finally {
