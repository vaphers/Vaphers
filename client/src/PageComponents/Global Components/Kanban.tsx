...
function getCardStateWeight(card: CardItem) {
  if (card.completed) return 0; // Completed at bottom
  if (card.deadline) {
    const parsed = parseISO(card.deadline);
    if (isValid(parsed) && differenceInDays(parsed, new Date()) < 0) {
      return 2; // Overdue at top
    }
  }
  return 1; // Active in middle
}

function sortCards(items: CardItem[], sortMode: SortMode): CardItem[] {
  return [...items].sort((a, b) => {
    // Primary Sort: Overdue (2) > Active (1) > Completed (0)
    const weightA = getCardStateWeight(a);
    const weightB = getCardStateWeight(b);
    if (weightA !== weightB) {
      return weightB - weightA; 
    }

    // Secondary Sort: Chosen Sort Mode
    if (sortMode === "priority") {
      return (PRIORITY_CONFIG[b.priority].weight) - (PRIORITY_CONFIG[a.priority].weight);
    }
    if (sortMode === "deadline") {
...
function KanbanCard({ card, index, colIndex, searchQuery, isSearchActive, onClick }: {
  ...
  if (!card.completed && ds) {
    if (ds.overdue) {
      isOverdue = true;
      cardBgClass = "bg-red-100";
    } else if (card.deadline) {
       const days = differenceInDays(parseISO(card.deadline), new Date());
       if (days >= 0 && days <= 3) {
    ...
    <Draggable draggableId={card.id} index={index} isDragDisabled={isSearchActive}>
      {(provided, snapshot) => (
        <div
          ...
          className={`${cardBgClass} cursor-pointer select-none transition-all duration-100 relative overflow-hidden
            ${snapshot.isDragging ? "shadow-xl" : "hover:shadow-md hover:border-slate-300"}
            ${card.completed ? "opacity-90" : ""}`}
        >
          {/* Overdue stamp */}
          {isOverdue && !card.completed && (
            <div className="absolute inset-0 pointer-events-none z-10 flex items-center justify-center overflow-hidden">
              <span style={{
                transform: "rotate(-28deg)",
                fontSize: "13px",
          ...
          {card.completed && (
            <div className="absolute inset-0 pointer-events-none z-10 flex items-center justify-center overflow-hidden">
              <div style={{
                position: "absolute",
                inset: 0,
                background: "repeating-linear-gradient(135deg, transparent, transparent 18px, rgba(4,120,87,0.06) 18px, rgba(4,120,87,0.06) 20px)",
              }} />
              <span style={{
                transform: "rotate(-28deg)",
                fontSize: "13px",
                  ...
                  {filteredItems.map(card => {
                    ...
                    if (!card.completed && ds) {
                        if (ds.overdue) {
                            ...
                            const days = differenceInDays(parseISO(card.deadline), new Date());
                            if (days >= 0 && days <= 3) bgClass = "bg-red-50 hover:bg-red-100";
                        }
                    }

                    return (
                      <div key={card.id} onClick={() => onEditCard(col.id, card)}
                        className={`relative overflow-hidden border-t border-slate-100 cursor-pointer transition-colors group ${bgClass}`}>
                        
                        {!card.completed && ds?.overdue && (
                          <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10 overflow-hidden">
                            <span className="text-[10px] font-black uppercase tracking-[0.25em] select-none"
                              style={{ transform: "rotate(-8deg)", opacity: 0.25, whiteSpace: "nowrap", color: "#dc2626", fontWeight: 900 }}>
                        ...
                        {card.completed && (
                          <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10 overflow-hidden">
                            <span className="text-[10px] font-black text-emerald-700 uppercase tracking-[0.25em] select-none"
                              style={{ transform: "rotate(-8deg)", opacity: 0.35, whiteSpace: "nowrap", color: "#047857", fontWeight: 900 }}>
                              ✓ DONE ✓ DONE ✓ DONE ✓ DONE
                            </span>
                          </div>
                        )}
